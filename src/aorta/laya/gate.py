...
* **The existing regex.** ``sanitizer_assessment()`` answers a machine-readable
  ...
  is unbeatable, so the useful figure is its *coverage*: a Laya tier is only
...
from aorta.laya.predictor import Calibration, LayaPredictor, Question
...
#: strategy clears. See :attr:`aorta.laya.eval.EvalResult.false_positive_rate`.
#:
...
def assert_measurable(predictor: LayaPredictor) -> None:
    ...
    careful about it. ``FakeLayaPredictor`` answers from a hash of its inputs,
...
class GateResult:
    ...
    #: :func:`aorta.laya.eval.fit_temperatures` derives on our corpus, which lands
...
def run_gate(
    ...
    *,
    ...
    The candidate goes through :func:`aorta.laya.eval.evaluate`, so its
    ...
    what :func:`aorta.laya.eval.split_examples` being deterministic buys: both

...
* ``bundle/logs/watch.stderr.log`` -- written by ``write_bundle(job, job_dir,
  ...
  is the state a Laya tier would read.
...
from aorta.laya.corpus.schema import BuildResult, CorpusError, LabelledExample, iter_json_lines
...
def watch_questions() -> tuple[Noul, Choice]:
    """The two questions Watch's Laya tier asks, from the tier's own definitions.
    ...
    **The assemblers, not the five strings.** Track B splits into strings because
    ...
    :class:`~aorta.laya.predictor.ChoiceAnswer` re-reads a distribution in the
    ...
    reaching it eagerly would make the whole ``aorta.laya`` package need the
    ...
    try:
        ...
        raise CorpusError(
            "the Watch corpus labels against the questions Watch's own Laya tier "

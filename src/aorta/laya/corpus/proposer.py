...
from aorta.laya.corpus.schema import BuildResult, LabelledExample, iter_json_lines
...
#: :mod:`aorta.laya.predictor` deliberately holds none of them: it serves three
...
#: ``aorta/agent/llm.py`` beside ``LAYA_MITIGATION_QUESTION`` when Track B
...
LAYA_CANDIDATE_WHEN_TRUE = "this mitigation addresses the cause and the repro would pass"
LAYA_CANDIDATE_WHEN_FALSE = "this mitigation is unrelated to the cause, or not enough on its own"
...
def candidate_question(mitigation: str) -> Noul:
    ...
    """
    return Noul(
        question=LAYA_CANDIDATE_QUESTION_TEMPLATE.format(mitigation=mitigation),
        when_true=LAYA_CANDIDATE_WHEN_TRUE,
        when_false=LAYA_CANDIDATE_WHEN_FALSE,
    )
        ...
        if winner is not None and winner in remaining:
            result.examples.append(
                LabelledExample(
                    ...
                    state=state,
                    question=Choice(
                        question=LAYA_MITIGATION_QUESTION,
...
def _add_candidate_nouls(
    ...
    **Both shapes, so Phase 1 measures which is better instead of arguing it.**
    ...
    ``tests/cia/test_laya_predictor.py`` asserts; each noul lands in the ``noul:2``

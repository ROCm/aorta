...
| [`rocjitsu_sanitizers`](rocjitsu_sanitizers/README.md) | Experimental typed kernel worklists, deterministic top-N selection, exact-entry static Waitcheck, fail-closed Record/Replay parsing, and versioned sanitizer reports. Recipe wiring, provisioning, and scoped ConSan are explicit follow-ups; ConSan never falls back to whole-application instrumentation. | `select_kernels(...)`, `run_waitcheck(...)`, `run_sanitizers(...)`, `SanitizerReport` |

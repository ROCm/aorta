"""``aorta laya`` -- build the decision corpora and measure a typed-decision model.

The only Click code for :mod:`aorta.laya`, which contains none, on the same split
``aorta chat`` uses and for the same reason.

**Module scope is click plus stdlib, deliberately.** Nothing in
:mod:`aorta.laya` pulls torch by itself, so the letter of the rule that
``aorta/cli/chat.py`` follows would not require deferring these imports -- but
the reason does. ``aorta laya eval`` resolves a checkpoint, the log-finder corpus
builder reaches into ``aorta.cia.watch`` and so into dspy, and neither belongs on
the critical path of ``aorta --help``. The same discipline also means a missing
extra is reported as a sentence rather than as a traceback, which is the whole
point of ``_load`` in the chat command.

Two commands, in the order Phase 0 and Phase 1 happen:

    aorta laya corpus watch --root ~/cia-jobs --output watch.jsonl
...
#: ``tests/cia/test_laya_cli.py`` fails if this list and ``corpus.BUILDERS``
#: drift apart.
_CORPUS_KINDS = ("watch", "proposer", "log-finder")

#: Likewise against ``aorta.laya.predictor.CHECKPOINTS``, plus ``fake``.
_BACKENDS = ("fake", "laya", "laya-typed-decisions")

_INSTALL_HINT = (
    "A real typed-decision predictor needs the laya extra.\n"
    "Install it with:  pip install 'amd-aorta[laya]'\n"
...
@laya.command(name="eval")
@click.option("--corpus", "corpus_path", required=True, help="Labelled JSONL from 'laya corpus'.")
@click.option(
    "--backend",
...
def eval_command(
    ...
    """
    _logging(verbose)
    from aorta.laya.eval import DEFAULT_HOLDOUT, load_corpus
    from aorta.laya.gate import run_gate

...
There is no `tokenspeed.py` workload class. These scripts are the whole
integration: the recipes run on the built-in `_subprocess` path (`mode: probe`),
which wraps an opaque command, so `host_launch.sh` is the only thing aorta sees.
...
| `harvest_code_objects.py` | host | Runs a kernel — via the benchmark harness or `--pytest-suite` — with a clean Triton cache, collects the JIT-compiled `.hsaco`, and emits a `mode: sanitizer` recipe pinning each object by SHA-256. `--pytest-suite` is the only route to attention and MoE code objects. |
...
Exit codes are a documented interface — the recipes' `custom_patterns` and
`tests/probe/test_tokenspeed_probe.py` both depend on them. See the header of
each script.

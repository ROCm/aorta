"""The decision-predictor seam: typed questions in, calibrated probabilities out.
...
The encoder behind this seam answers typed questions instead -- the current
implementation is the Laya model, loaded by :class:`LayaAgentPredictor`. A
``noul`` is a yes/no whose answer is p(yes); a ``choice`` is one of N named
options with a probability each. Both are trained against strictly proper
scoring rules, both come out of one forward pass, and neither generates a token
-- so there is nothing to parse and no path by which an answer can name an
option that was not offered. That last property is the security argument for
the log finder as much as the cost argument.
...
resolves a backend name in one factory. A ``LocalClassifierProposer`` is going to arrive as
...
**``score`` is deliberately absent.** The Laya library has a third primitive that returns a
...
#: question, and ``tests/cia/test_local_classifier_predictor.py`` pins it against the boundary
...
#: the published wheel. Kept equal to the floor in ``pyproject.toml``'s ``[local-classifier]``
#: extra, and asserted so by ``tests/cia/test_local_classifier_predictor.py``.
#:
...
class NoulAnswer:
    ...
    existing ``confidence_threshold: 0.70`` and the ``clean_threshold`` a local-classifier
...
def bucket_for(question: Question) -> str:
    ...
    is :meth:`DecisionPredictor.calibration`'s job. Conflating the two is what makes a
    ...
    Not the same thing as :func:`aorta.local_classifier.eval.calibration_key`, which keys on
...
class DecisionPredictor(Protocol):
...
def ask_one(
    predictor: DecisionPredictor, state: str, questions: Sequence[Question]
) -> list[Answer]:
    """Ask several questions about one state. One forward pass."""
    return predictor.ask([state], questions)[0]


def ask_noul(predictor: DecisionPredictor, state: str, question: Noul) -> NoulAnswer:
...
def ask_choice(predictor: DecisionPredictor, state: str, question: Choice) -> ChoiceAnswer:
...
class LayaAgentPredictor:
    ...
    *load* is the injection point. It defaults to the library's own
    ...
    def token_count(self, text: str) -> int:
        ...
        Deliberately absent from :class:`FakeDecisionPredictor`, so a census cannot

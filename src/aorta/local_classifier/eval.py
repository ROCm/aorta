...
``tests/cia/test_local_classifier_scoring.py``, and :func:`evaluate` -- the part that needs
...
**A number from a fake predictor is not a measurement.** :class:`FakeDecisionPredictor`
...
#: anybody runs.** Watch's local-classifier tier skips the LLM when ``p(healthy)`` reaches
#: ``watch.local_classifier.clean_threshold``; the probe agent stops when ``p(stop)`` reaches
#: ``DEFAULT_CLASSIFIER_STOP_THRESHOLD``. Scoring those on accuracy measures a decision
...
#: ``tests/cia/test_local_classifier_scoring.py`` fails if a number here and the shipped one
...
#: the same kind of number as ``DEFAULT_CLASSIFIER_STOP_THRESHOLD`` and carries the same
...
def calibration_key(example: LabelledExample) -> str:
    ...
    handing the probabilities back, which is why the ``[local-classifier]`` extra floors at
...
class EvalResult:
    ...
    def false_positive_rate(self) -> float:
        ...
        **Accuracy is not merely imprecise here, it is unstable in a way that
        ...
        Lower is better, which is why ``aorta.local_classifier.gate.LOWER_IS_BETTER`` exists.
        ...
        ``tests/cia/test_local_classifier_gate.py`` rather than left to a sentinel.
        """
...
def answer_all(
    predictor: DecisionPredictor, examples: Sequence[LabelledExample]
) -> list[Distribution]:
...
def load_corpus(path: str) -> list[LabelledExample]:
    ...
    """
    from aorta.local_classifier.corpus.schema import read_corpus

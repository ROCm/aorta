...
from aorta.local_classifier.predictor import Choice, Noul, Question
...
class LabelledExample:
    ...
    *label* is an option name: ``"true"`` / ``"false"`` for a noul, and one of
    ...
    #: lets :attr:`aorta.local_classifier.eval.EvalResult.group_top1` score the choice that was
    #: actually being made.
    #:
...
def read_corpus(path: str | Path) -> list[LabelledExample]:
    ...
    try:
        ...
        raise CorpusError(
            f"no corpus at {target}. Build one first with "
            "'python -m aorta.local_classifier corpus <kind>'."
        ) from exc

"""Typed, calibrated decisions from a local encoder, and the offline
measurement that decides whether they are worth integrating.

The encoder is an implementation detail behind :class:`DecisionPredictor`.
The current implementation is the Laya model, loaded by
:class:`LayaAgentPredictor`; nothing outside that class depends on it.

Three things live here, in the order they have to happen:

* :mod:`aorta.local_classifier.predictor` -- the seam. A narrow Protocol, a fake, and a
  ...
  contract.
* :mod:`aorta.local_classifier.corpus` -- three builders that turn artifacts already on
  disk into labelled JSONL. Nothing downstream is measurable without them.
* :mod:`aorta.local_classifier.eval` and :mod:`aorta.local_classifier.gate` -- the scoring, which is

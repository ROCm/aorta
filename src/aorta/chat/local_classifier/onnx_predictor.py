"""A :class:`~aorta.local_classifier.predictor.DecisionPredictor` on onnxruntime, with no torch.

This is the chat path's implementation of the seam every other track reaches
through :class:`~aorta.local_classifier.predictor.LayaAgentPredictor`. Same Protocol, same
:class:`~aorta.local_classifier.predictor.Noul` and
:class:`~aorta.local_classifier.predictor.Choice` in, same
:class:`~aorta.local_classifier.predictor.NoulAnswer` and
:class:`~aorta.local_classifier.predictor.ChoiceAnswer` out -- so ``router_node`` and
``selector_node`` are written against the contract rather than against this
class, and their tests run on ``FakeDecisionPredictor`` with no artifact anywhere
near the machine.
...
:class:`~aorta.chat.local_classifier.artifact.ArtifactUnavailableError` rather than returning a
...
**What has never been run.** There is no fine-tuned checkpoint and no
classifier weights on the machine this was written on, so the loaders, the graph signature
...
failure message. See ``tests/chat/test_local_classifier_onnx_predictor.py``.
"""
...
#: neither. Quoted from :mod:`aorta.local_classifier.predictor`'s ``_as_library_question``,
...
def answers_from_scores(
    ...
    through :func:`aorta.local_classifier.eval.apply_temperature`, rather than by dividing
    ...
    """
    from aorta.local_classifier.eval import apply_temperature
...
class OnnxDecisionPredictor:
    """An exported classifier artifact on onnxruntime. Loaded on first use.

    Loading is deferred to the first :meth:`ask` for the reason
    :class:`~aorta.local_classifier.predictor.LayaAgentPredictor` defers its own: one
    ...
    *session_factory* and *tokenizer_factory* are the injection points, the same
    ...
    def model_id(self) -> str:
        ...
        try:
            ...
            return f"onnx:{self._directory}(unloadable)"
    ...
    def _tokenizer_for(self, artifact: Artifact) -> Any:
        ...
        try:
            ...
            raise ArtifactUnavailableError(
                "the local-classifier ONNX path needs the 'tokenizers' package, which arrives "
    ...
    def _session_for(self, artifact: Artifact) -> Any:
        ...
        try:
            ...
            raise ArtifactUnavailableError(
                "the local-classifier ONNX path needs onnxruntime, which arrives with fastembed "
    ...
    def _run(self, session: Any, ids: list[int], positions: list[int]) -> list[float]:
        ...
        try:
            ...
            raise ArtifactUnavailableError(
                f"the exported classifier graph failed to run: {type(exc).__name__}: {exc}"
            ) from exc
        if not outputs:
            raise ArtifactUnavailableError(
                f"the exported classifier graph returned nothing for {GRAPH_OUTPUT!r}"
            )

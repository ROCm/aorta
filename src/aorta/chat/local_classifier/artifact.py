"""What an exported classifier artifact is, and which weights and fit produced it.
...
Decision 22 (``docs/local-classifier-packaging.md``) says what has to be written down, and
...
inside :mod:`aorta.chat.local_classifier.onnx_predictor`'s loaders, because ``onnxruntime``
...
from aorta.local_classifier.predictor import ClassifierUnavailableError, Noul, Question
...
MANIFEST_NAME = "aorta-local-classifier-onnx.json"
...
STAGING_PROCEDURE = (
    "No exported classifier artifact at {path}, so the chat router and selector "
    "cannot run on the local classifier.\n"
    "\n"
    ...
    "  export AORTA_CHAT_LOCAL_CLASSIFIER_ARTIFACT_PATH={path}\n"
    "\n"
    "Until then, leave AORTA_CHAT_LOCAL_CLASSIFIER_ENABLED unset: the router and selector\n"
    "keep running on the LLM, which is what they do today."
)


class ArtifactUnavailableError(ClassifierUnavailableError):
    ...
    ``except``. The chat nodes have exactly that shape: the classifier is a tier, and
...
def calibration_key(question: Question) -> str:
    ...
    The same convention as :func:`aorta.local_classifier.eval.calibration_key`, which keys
    ...
    ``tests/chat/test_local_classifier_artifact.py`` asserts the two agree.
    """
...
class RenderTemplate:
    ...
    **This is captured from the source checkpoint at export time and never
    ...
    So :mod:`aorta.chat.local_classifier.export` reads these fields off the checkpoint's own
    ...
    *marker* is the token that precedes every option. It has to be a single
    token in the checkpoint's tokenizer, which :mod:`~aorta.chat.local_classifier.onnx_predictor`
...
class Manifest:
    ...
    def identity(self) -> str:
        """What :meth:`OnnxDecisionPredictor.model_id` reports, inline in every report.
    ...
    def from_dict(cls, raw: Any) -> Manifest:
        ...
        if missing:
            raise ArtifactUnavailableError(
                ...
                "(see docs/local-classifier-packaging.md, Rule 2)."
            )

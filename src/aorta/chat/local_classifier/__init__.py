"""The local classifier on the chat path: the ONNX runtime, its artifact, and the two questions.

Four modules, in the order they matter to a reader:

* :mod:`~aorta.chat.local_classifier.questions` -- the exact text ``router_node`` and
  ``selector_node`` ask, in one place, so nothing re-phrases it.
* :mod:`~aorta.chat.local_classifier.artifact` -- what an exported artifact is, and which
  weights and which temperature fit produced it.
* :mod:`~aorta.chat.local_classifier.onnx_predictor` -- the
  :class:`~aorta.local_classifier.predictor.DecisionPredictor` implementation that runs it.
* :mod:`~aorta.chat.local_classifier.export` -- the torch-side tool that produces one.

**Why this package is under** ``aorta/chat/`` **and not under** ``aorta/local_classifier/``.
Decision 22 splits the integration by runtime: the CIA tracks run torch out of
the ``[local-classifier]`` extra, and the chat path runs ONNX on the onnxruntime
...
loader in the core ``aorta.local_classifier`` package would either drag a chat dependency
into core or force ``aorta.local_classifier`` to import ``aorta.chat`` -- which
...
:mod:`aorta.local_classifier.predictor`'s Protocol and types, and nothing in core reads
anything here.
...
#: locks its model.
_predictor: DecisionPredictor | None = None
...
def chat_predictor() -> DecisionPredictor:
    ...
    :class:`~aorta.local_classifier.predictor.FakeDecisionPredictor` from configuration, even
    ...
    Raises :class:`~aorta.chat.local_classifier.artifact.ArtifactUnavailableError` when the
    artifact is absent or unreadable. Callers treat that as "no local-classifier tier" and
    ...
    if _predictor is None:
        with _predictor_lock:
            if _predictor is None:
                ...
                from aorta.chat.local_classifier.onnx_predictor import OnnxDecisionPredictor

                _predictor = OnnxDecisionPredictor(
                    settings.local_classifier_artifact_path,
                    verify_digest=settings.local_classifier_verify_digest,
                )
    return _predictor

...
allowed -- an ``[local-classifier]`` install, on whatever machine holds the fine-tune -- and
writes a directory that :mod:`aorta.chat.local_classifier.onnx_predictor` can load on a
...
:class:`~aorta.local_classifier.predictor.LayaAgentPredictor` keeps.

**It lives under** ``aorta/chat/`` **rather than under** ``aorta/local_classifier/``
**because the artifact it writes is the chat path's format.** The schema in
:mod:`aorta.chat.local_classifier.artifact` -- the graph signature, the marker convention,
...
:mod:`aorta.local_classifier.predictor` is fine and an exporter under ``aorta/local_classifier/``
...
digests -- are unit-tested against fakes in ``tests/chat/test_local_classifier_export.py``.
...
def resolve_modules(agent: Any) -> tuple[Any, Any]:
    ...
    if encoder is None or head is None:
        ...
        raise ExportError(
            ...
            "aorta/chat/local_classifier/export.py rather than exporting the wrong subgraph."
        )
    return encoder, head
...
def check_temperatures(temperatures: Mapping[str, Any]) -> dict[str, float]:
    ...
    harness has run, and :meth:`~aorta.chat.local_classifier.artifact.Manifest.fit_id`
...
def write_manifest(directory: str | Path, manifest: Manifest) -> Path:
    ...
    Last, deliberately. :func:`~aorta.chat.local_classifier.artifact.load_artifact` keys off
...
def export_artifact(
    ...
    *load* and *graph_writer* are the injection points, mirroring
    :class:`~aorta.local_classifier.predictor.LayaAgentPredictor`'s ``load=``. They are
...
def _load_agent(checkpoint: str, *, device: str, load: Any) -> tuple[Any, str]:
    ...
    Goes through :class:`~aorta.local_classifier.predictor.LayaAgentPredictor` rather than
    ...
    """
    from aorta.local_classifier.predictor import LayaAgentPredictor
...
def write_torch_graph(
    ...
    **This is the one function in this module that has never been run.** It
    ...
    Python in :mod:`~aorta.chat.local_classifier.onnx_predictor` -- testable with no

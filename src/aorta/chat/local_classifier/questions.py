"""The exact questions the chat graph asks the local classifier, and the thresholds it reads.

**One definition, because the alternative has already happened once.** Track B
found that the corpus builder under ``src/aorta/local_classifier/corpus/`` had independently
...
``aorta/local_classifier/corpus/`` cannot reach in here. That is a real problem for exactly
the defect above, and the honest options are to move these constants into
``aorta/local_classifier/`` once a builder needs them, or to make the corpus package a
sanctioned importer. It is recorded here rather than solved because both options
edit a package this change does not own.
``tests/chat/test_local_classifier_questions.py`` guards the interim with a test that fails
...
``tests/chat/test_local_classifier_questions.py`` asserts both halves, including the
...
#: there is one -- see ``docs/local-classifier-packaging.md`` on a probability being a

"""The selector's local-classifier tier: one noul per tool, one forward pass, still advisory.
...
So the tests below assert the property from both ends: that a local-classifier
failure produces the same empty list an LLM failure produces, and that an empty list
means the act node binds the whole registry and is shown no narrowing message.

Everything here runs against ``FakeDecisionPredictor`` with pinned answers. It has
...
from aorta.local_classifier.predictor import NoulAnswer
...
class ScriptedPredictor:
    ...
    A recorder rather than ``FakeDecisionPredictor`` for the tests that are about
...
@pytest.fixture()
def local_classifier_on(monkeypatch):
    """Turn the tier on and install a predictor. See ``test_local_classifier_router.py``."""
    from aorta.chat.config import settings

    monkeypatch.setattr(settings, "local_classifier_enabled", True)

    def _install(predictor):
        monkeypatch.setattr("aorta.chat.local_classifier.chat_predictor", lambda: predictor)
        return predictor
...
class TestTheRanking:
    async def test_the_tools_come_back_best_first(self, local_classifier_on):
        local_classifier_on(ScriptedPredictor({"search_code": 0.7, "grep_code": 0.9, "read_file": 0.8}))
        out = await _select()
        assert out["candidate_tools"] == ["grep_code", "read_file", "search_code"]

    async def test_the_shortlist_is_capped(self, local_classifier_on):
        local_classifier_on(ScriptedPredictor(dict.fromkeys(_described(), 0.9)))
        out = await _select()
        assert len(out["candidate_tools"]) == MAX_CANDIDATES

    async def test_a_tool_below_the_threshold_is_not_recommended(self, local_classifier_on, monkeypatch):
        ...
        monkeypatch.setattr(settings, "local_classifier_selector_threshold", 0.5)
        local_classifier_on(ScriptedPredictor({"grep_code": 0.9, "search_code": 0.49}))
        out = await _select()
        assert out["candidate_tools"] == ["grep_code"]

    async def test_nothing_above_the_threshold_recommends_nothing(self, local_classifier_on):
        local_classifier_on(ScriptedPredictor({}, default=0.01))
        out = await _select()
        assert out["candidate_tools"] == []
        assert "no tool reached" in out["selection_rationale"]

    async def test_the_rationale_names_the_model_and_the_probabilities(self, local_classifier_on):
        ...
        """
        local_classifier_on(ScriptedPredictor({"grep_code": 0.93}))
...
class TestTheBatchingShape:
    async def test_every_tool_is_one_question_in_one_forward_pass(self, local_classifier_on):
        ...
        """
        predictor = local_classifier_on(ScriptedPredictor({"grep_code": 0.9}))
        await _select()
    ...
    async def test_no_llm_is_consulted_when_local_classifier_answers(self, local_classifier_on):
        local_classifier_on(ScriptedPredictor({"grep_code": 0.9}))
...
class TestItStaysAdvisory:
    ...
    async def test_a_failing_predictor_falls_back_to_the_llm(self, local_classifier_on):
        class Broken:
            def model_id(self):
                return "broken"
    ...
    async def test_both_tiers_failing_widens_to_every_tool(self, local_classifier_on):
    ...
    async def test_a_predictor_answering_the_wrong_shape_falls_back(self, local_classifier_on):
        """A noul answered with a distribution is a contract break, not a ranking."""
        from aorta.local_classifier.predictor import ChoiceAnswer
        ...
        local_classifier_on(WrongShape())
        llm = MagicMock()
    ...
    async def test_a_predictor_returning_a_non_iterable_falls_back(self, local_classifier_on):
        ...
        local_classifier_on(NotIterable())
        llm = MagicMock()
        llm.ainvoke = AsyncMock(return_value=AIMessage(content="not json"))
        with patch("aorta.chat.graph.nodes._get_llm", return_value=llm):
            out = await _select()
    ...
    async def test_a_predictor_answering_fewer_nouls_than_asked_falls_back(
        self, local_classifier_on, caplog
    ):
    ...
    async def test_a_predictor_that_cannot_name_itself_still_ranks(self, local_classifier_on):
        ...
        local_classifier_on(Nameless({"grep_code": 0.97}))
        out = await _select()
        assert out["candidate_tools"] == ["grep_code"]
    ...
    async def test_the_act_node_binds_every_tool_whatever_the_selector_said(self, local_classifier_on):
        """The end of the invariant, where it would actually bite.

        ``_act_native`` binds ``TOOL_REGISTRY`` and not the shortlist. Asserted
        against a local-classifier ranking that named exactly one tool, because the failure
        ...
        the classifier ranked fourth has no way to reach it.
        """
        from aorta.chat.graph import nodes

        local_classifier_on(ScriptedPredictor({"grep_code": 0.99}))
        out = await _select()
        assert out["candidate_tools"] == ["grep_code"]
...
class TestTheStructuralFilterStillApplies:
    async def test_a_tool_needing_a_paste_is_dropped_when_nothing_was_pasted(
        self, local_classifier_on, monkeypatch
    ):
        ...
        local_classifier_on(ScriptedPredictor({"read_file": 0.99, "grep_code": 0.8}))
        out = await _select(_PROSE)
        assert out["candidate_tools"] == ["grep_code"]

    async def test_dropping_is_explained_rather_than_silent(self, local_classifier_on, monkeypatch):
        from aorta.chat.tools import capabilities

        monkeypatch.setattr(
            capabilities, "_REQUIRES", {"read_file": frozenset({"pasted_source"})}
        )
        local_classifier_on(ScriptedPredictor({"read_file": 0.99}))
    ...
    async def test_the_same_tool_survives_when_source_is_present(self, local_classifier_on, monkeypatch):
        from aorta.chat.tools import capabilities

...
There is no Laya checkpoint and no exported artifact on the machine this was
...
from aorta.chat.laya.onnx_predictor import (
...
from aorta.laya.predictor import Choice, ChoiceAnswer, LayaPredictor, Noul, NoulAnswer
...
class TestThePredictor:
    def test_it_satisfies_the_protocol(self, tmp_path):
        ...
        checked: LayaPredictor = predictor
...
class TestTheModelId:
    def test_it_names_the_checkpoint_the_graph_and_the_fit(self, tmp_path):
        predictor, _ = _predictor(tmp_path, [0.0, 1.0])
        assert predictor.model_id().startswith("laya-onnx:fine-tune@")

"""The router's local-classifier tier: a threshold where a sentence used to be.

``ROUTER_PROMPT`` ends with "If in doubt, classify as action", which is a
calibration instruction written in English. The local-classifier tier replaces it with
:data:`~aorta.chat.local_classifier.questions.DEFAULT_ROUTER_THRESHOLD`, which is a number
...
The two fallback constants are not tested here because the local-classifier path has no use
...
from aorta.local_classifier.predictor import FakeDecisionPredictor, NoulAnswer
...
def local_classifier_on(monkeypatch):
    ...
    The factory is patched on ``aorta.chat.local_classifier`` rather than on the node,
...
def _pinned(probability: float) -> FakeDecisionPredictor:
...
class TestTheThresholdDecides:
    async def test_a_confident_yes_is_the_action_branch(self, local_classifier_on):
        local_classifier_on(_pinned(0.95))
        assert (await _route())["route"] == "action"

    async def test_a_confident_no_is_the_question_branch(self, local_classifier_on):
        local_classifier_on(_pinned(0.02))
        assert (await _route())["route"] == "question"

    async def test_at_the_threshold_counts_as_reaching_it(self, local_classifier_on, monkeypatch):
        ...
        monkeypatch.setattr(settings, "local_classifier_router_threshold", 0.4)
        local_classifier_on(_pinned(0.4))
        assert (await _route())["route"] == "action"

    async def test_the_threshold_is_what_moves_the_answer(self, local_classifier_on, monkeypatch):
        """The whole claim: the decision is a number, not a prompt."""
        from aorta.chat.config import settings

        local_classifier_on(_pinned(0.3))
        monkeypatch.setattr(settings, "local_classifier_router_threshold", 0.2)
        assert (await _route())["route"] == "action"
        monkeypatch.setattr(settings, "local_classifier_router_threshold", 0.6)
        assert (await _route())["route"] == "question"

    async def test_the_default_threshold_keeps_an_uncertain_turn_on_the_action_branch(
        self, local_classifier_on
    ):
        ...
        answers.
        """
        local_classifier_on(_pinned(0.45))
        assert (await _route())["route"] == "action"
    ...
    async def test_no_llm_is_consulted_when_local_classifier_answers(self, local_classifier_on):
        ...
        local_classifier_on(_pinned(0.9))
...
class TestItFallsBackToTheLLM:
    ...
    async def test_a_predictor_that_cannot_load_falls_back(self, monkeypatch):
        ...
        from aorta.chat.config import settings
        from aorta.chat.local_classifier.artifact import ArtifactUnavailableError
        ...
        monkeypatch.setattr("aorta.chat.local_classifier.chat_predictor", _no_artifact)
        llm = MagicMock()
        llm.ainvoke = AsyncMock(return_value=AIMessage(content="action"))
        with patch("aorta.chat.graph.nodes._get_llm", return_value=llm):
            assert (await _route())["route"] == "action"

    async def test_a_predictor_that_raises_mid_question_falls_back(self, local_classifier_on):
...
class TestWhatItSaysAboutItself:
    async def test_the_log_names_the_model_and_the_probability(self, local_classifier_on, caplog):
        """Decision 22: a verdict that cannot name what produced it is not evidence."""
        import logging

        local_classifier_on(_pinned(0.91))
    ...
    async def test_a_predictor_that_cannot_name_itself_still_routes(self, local_classifier_on, caplog):
        ...
        class Nameless(FakeDecisionPredictor):
            def model_id(self):
                raise RuntimeError("the manifest went away")

        local_classifier_on(Nameless(pinned={ROUTER_QUESTION.question: NoulAnswer(probability=0.95)}))
        with caplog.at_level(logging.INFO, logger="aorta.chat.graph.nodes"):
            assert (await _route())["route"] == "action"

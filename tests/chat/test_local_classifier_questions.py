...
builder under ``src/aorta/local_classifier/corpus/`` had independently written its own
...
from aorta.chat.local_classifier.artifact import calibration_key
...
_QUESTIONS_MODULE = _SRC / "chat" / "local_classifier" / "questions.py"

#: Where Laya's own high-cardinality calibration gets bad, per the model card
#: and the ``[local-classifier]`` floor's reasoning in ``pyproject.toml``.
_CHOICE_CLIFF = 11
...
class TestEachQuestionIsWrittenOnce:
    ...
    def test_it_appears_in_exactly_one_module(self, text: str):
        ...
        assert found == [_QUESTIONS_MODULE], (
            ...
            "aorta.chat.local_classifier.questions instead. Asking one wording and "
...
class TestNothingHereIsAWideChoice:
    ...
    def test_a_single_choice_would_cross_the_cliff_on_a_full_install(self):
        ...
        assert len(full_surface) >= _CHOICE_CLIFF, (
            ...
            "aorta/chat/local_classifier/questions.py no longer applies and should be "

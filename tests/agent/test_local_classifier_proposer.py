...
Nothing here loads weights. ``FakeDecisionPredictor`` answers from a hash of its
...
``aorta/local_classifier/corpus/proposer.py`` recorded when it built the training corpus.
...
class Recording(FakeDecisionPredictor):
...
def _pinned(*, stop, mitigation=None, category=None, model_id="laya-typed-decisions"):
    ...
    """
    answers = {CLASSIFIER_STOP_QUESTION: NoulAnswer(probability=stop)}
    if mitigation is not None:
        answers[CLASSIFIER_MITIGATION_QUESTION] = ChoiceAnswer(probabilities=mitigation)
    if category is not None:
        answers[CLASSIFIER_CATEGORY_QUESTION] = ChoiceAnswer(probabilities=category)
...
class TestMakeProposer:
    def test_local_classifier_resolves_to_the_local_classifier_proposer(self):
        assert isinstance(make_proposer("local"), LocalClassifierProposer)
    ...
    def test_it_defaults_to_the_fine_tuned_checkpoint(self):
        ...
        assert make_proposer("local")._checkpoint == DEFAULT_CLASSIFIER_CHECKPOINT
        assert DEFAULT_CLASSIFIER_CHECKPOINT == "laya-typed-decisions"

    def test_llm_model_names_a_checkpoint_on_this_backend(self):
        """Not a model on someone's endpoint: local is an encoder, not a provider."""
        proposer = make_proposer("local", model="/srv/weights/rocm-finetune")
        assert proposer._checkpoint == "/srv/weights/rocm-finetune"

    def test_building_the_proposer_resolves_no_weights(self):
        """The loop builds a proposer before it knows whether it needs one."""
        assert make_proposer("local", model="not-a-real-checkpoint")._predictor is None
...
class TestTheImportBoundary:
    ...
    def test_an_exhausted_search_needs_neither_weights_nor_the_extra(self):
        proposer = LocalClassifierProposer(checkpoint="not-a-real-checkpoint")
    ...
    def test_constructing_one_imports_nothing(self, monkeypatch):
        ...
        assert LocalClassifierProposer(checkpoint="laya")._checkpoint == "laya"
...
class TestTheQuestions:
    def test_one_state_and_three_questions_in_one_forward_pass(self):
        ...
        """
        predictor = Recording()
        _propose(LocalClassifierProposer(predictor=predictor))
    ...
    def test_the_mitigation_choice_offers_exactly_what_is_left_to_try(self):
        ...
        predictor = Recording()
        _propose(LocalClassifierProposer(predictor=predictor), tried=["tf32_off"])
        mitigation = next(
            q for q in predictor.calls[0][1] if q.question == CLASSIFIER_MITIGATION_QUESTION
        )
    ...
    def test_the_category_choice_offers_the_derived_probe_set(self):
        ...
        _propose(LocalClassifierProposer(predictor=predictor))
        category = next(
            q for q in predictor.calls[0][1] if q.question == CLASSIFIER_CATEGORY_QUESTION
        )
    ...
    def test_the_stop_question_is_a_glossed_noul(self):
        ...
        predictor = Recording()
        _propose(LocalClassifierProposer(predictor=predictor))
        stop = next(q for q in predictor.calls[0][1] if q.question == CLASSIFIER_STOP_QUESTION)
        assert isinstance(stop, Noul)
        assert stop.when_true == CLASSIFIER_STOP_WHEN_TRUE
        assert stop.when_false == CLASSIFIER_STOP_WHEN_FALSE
    ...
    def test_the_questions_match_the_ones_the_corpus_labels(self):
        ...
        ``aorta/local_classifier/corpus/proposer.py`` now imports all three from here, so
        ...
        """
        from aorta.local_classifier.corpus import proposer as corpus

        assert corpus._STOP.question == CLASSIFIER_STOP_QUESTION
        assert corpus._STOP.when_true == CLASSIFIER_STOP_WHEN_TRUE
        assert corpus._STOP.when_false == CLASSIFIER_STOP_WHEN_FALSE
        assert corpus.CLASSIFIER_MITIGATION_QUESTION is CLASSIFIER_MITIGATION_QUESTION
        assert corpus.CLASSIFIER_CATEGORY_QUESTION is CLASSIFIER_CATEGORY_QUESTION
...
class TestTheProposedStep:
    def test_a_proposal_names_the_most_probable_candidate(self):
        ...
        step = _propose(LocalClassifierProposer(predictor=predictor))
    ...
    def test_confidence_is_the_probability_of_the_mitigation_proposed(self):
        ...
        assert _propose(LocalClassifierProposer(predictor=predictor)).confidence == pytest.approx(0.6)

    def test_the_category_comes_from_the_category_head(self):
        predictor = _pinned(stop=0.1, category=(("illegal_mem", 0.8), ("rccl_hang", 0.2)))
        assert _propose(LocalClassifierProposer(predictor=predictor)).category == "illegal_mem"
    ...
    def test_one_mitigation_is_proposed_at_a_time(self):
        ...
        step = _propose(LocalClassifierProposer(predictor=_pinned(stop=0.0)))
    ...
    def test_a_mitigation_outside_the_candidate_list_cannot_come_back(self, symptom):
        ...
        """
        step = _propose(
            LocalClassifierProposer(predictor=_pinned(stop=0.0)), tried=["tf32_off"], symptom=symptom
        )
    ...
    def test_the_hypothesis_follows_the_fake_proposers_template(self):
        """The classifier never emits text, so there is nothing to ask it for."""
        predictor = _pinned(
    ...
    def test_the_checkpoint_that_answered_is_recorded_in_the_step(self):
        ...
        """
        predictor = _pinned(stop=0.1, model_id="rocm-finetune@cpu")
        assert "rocm-finetune@cpu" in _propose(LocalClassifierProposer(predictor=predictor)).hypothesis
...
class TestTheDisclosure:
    ...
    *checkpoint*: ``Calibration.caveat`` reads the checkpoint's own shipped and
    ...
    def test_the_bucket_is_named_offline_even_when_nothing_is_known(self):
        """``FakeDecisionPredictor`` reports unknown, and the width half still lands."""
        wide = ["none"] + [f"mitigation_{n}" for n in range(21)]
        step = _propose(LocalClassifierProposer(predictor=_pinned(stop=0.0)), candidates=wide)
    ...
    def test_the_real_registry_lands_in_the_widest_bucket(self):
        ...
        step = _propose(
            LocalClassifierProposer(predictor=_pinned(stop=0.0)), candidates=["none", *candidates]
        )
    ...
    def test_a_clamped_bucket_is_disclosed_with_both_temperatures(self):
        step = _propose(
            LocalClassifierProposer(predictor=_clamping("choice:11+")),
    ...
    def test_a_clean_bucket_says_nothing(self):
        ...
        step = _propose(
            LocalClassifierProposer(predictor=_clamping("choice:3-5")),
            candidates=["none"] + [f"mitigation_{n}" for n in range(21)],
        )
    ...
    def test_a_clamped_noul_is_disclosed_on_the_step_that_thresholded_it(self):
        ...
        """
        step = _propose(LocalClassifierProposer(predictor=_clamping("noul:2", stop=0.99)))
        assert step.stop is True
        assert "NOT CALIBRATED" in step.hypothesis
    ...
    def test_the_calibration_is_read_after_the_answers_not_before(self):
        ...
        _propose(
            LocalClassifierProposer(predictor=Ordered(pinned={CLASSIFIER_STOP_QUESTION: NoulAnswer(0.0)}))
        )
    ...
    def test_the_category_choice_has_its_own_bucket(self):
        ...
        """
        from aorta.local_classifier.predictor import Choice, bucket_for
    ...
    def test_a_warning_raised_while_answering_is_not_swallowed(self):
        ...
        noisy = WarnsOnLoad(pinned={CLASSIFIER_STOP_QUESTION: NoulAnswer(probability=0.0)})
        with pytest.warns(RuntimeWarning, match="choice:11"):
            _propose(LocalClassifierProposer(predictor=noisy))
...
class TestStopping:
    def test_it_stops_once_the_noul_reaches_the_threshold(self):
        step = _propose(LocalClassifierProposer(predictor=_pinned(stop=0.9), stop_threshold=0.75))
        assert step.stop is True
        assert step.next_mitigations == []

    def test_it_keeps_going_below_the_threshold(self):
        step = _propose(LocalClassifierProposer(predictor=_pinned(stop=0.74), stop_threshold=0.75))
        assert step.stop is False
        assert step.next_mitigations

    def test_at_the_threshold_counts_as_reaching_it(self):
        """``>=``, the same reading ``should_alert`` uses on the other side of the tree."""
        step = _propose(LocalClassifierProposer(predictor=_pinned(stop=0.75), stop_threshold=0.75))
        assert step.stop is True
    ...
    def test_the_default_threshold_is_above_the_midpoint(self):
        ...
        """
        assert DEFAULT_CLASSIFIER_STOP_THRESHOLD > 0.5
    ...
    def test_a_stop_says_why_rather_than_leaving_the_loop_to_read_the_prose(self):
        ...
        step = _propose(LocalClassifierProposer(predictor=_pinned(stop=0.99)))
    ...
    def test_a_stop_reports_the_probability_and_the_threshold_it_stopped_on(self):
        step = _propose(LocalClassifierProposer(predictor=_pinned(stop=0.92), stop_threshold=0.75))
    ...
    def test_an_exhausted_candidate_list_stops_before_the_model_is_consulted(self):
        ...
        predictor = Recording()
        step = _propose(
            LocalClassifierProposer(predictor=predictor),
...
class TestFailures:
    def test_a_missing_checkpoint_is_loud_rather_than_a_silent_stop(self):
        ...
        with pytest.raises(ClassifierUnavailableError, match="unknown Laya checkpoint"):
            _propose(LocalClassifierProposer(checkpoint="laya-multilingual"))
    ...
    def test_a_predictor_answering_the_wrong_shape_is_not_read_as_a_probability(self):
        ...
        with pytest.raises(TypeError, match="answered the proposer's three questions"):
            _propose(LocalClassifierProposer(predictor=Wrong()))
    ...
    def test_the_weights_are_resolved_once_across_iterations(self, monkeypatch):
        ...
        """
        import aorta.local_classifier.predictor as seam
        ...
        proposer = LocalClassifierProposer(checkpoint="laya-typed-decisions")
        _propose(proposer)
        _propose(proposer)
...
class TestTheProtocol:
    def test_it_is_an_llm_proposer(self):
        ...
        proposer: LLMProposer = LocalClassifierProposer(predictor=_pinned(stop=0.0))

...
def test_the_archived_demo_trajectories_parse_unchanged():
    """The two runs already archived for the RL harvest, read as-is.
    ...
    expected = {
        archive / "live" / "DEMO-JOINT-LIVE": ("litellm", "pytorch_no_cuda_memory_caching"),
        archive / "fake" / "DEMO-JOINT-FAKE": ("fake", None),
    }
    present = {d: v for d, v in expected.items() if (d / "agent_log.jsonl").is_file()}
    if not present:
        pytest.skip(f"no archived agent_log.jsonl under {archive}")
    for run_dir, (backend, winner) in present.items():
        records = _records((run_dir / "agent_log.jsonl").read_text(encoding="utf-8"))
        assert records[0]["llm_backend"] == backend
        # Written by a code path that predates the key, so it cannot be there.
        assert not [r for r in records if "unresolved_mitigations" in r]
        state = wake(run_dir, ticket=run_dir.name)
        assert state.winning_mitigation == winner

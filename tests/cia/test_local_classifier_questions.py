"""One definition of each question the CIA tracks ask the local classifier.

The CIA counterpart of ``tests/chat/test_local_classifier_questions.py``, and it exists
...
**What goes wrong.** A local-classifier probability is calibrated by a temperature fitted per
...
The seam in ``aorta.local_classifier.predictor`` deliberately holds none of them -- it serves
...
``aorta.local_classifier.corpus`` must stay importable without the ``[cia]`` extra and
...
from aorta.cia.watch.log_finder import (
    ...
    CLASSIFIER_USEFUL_WHEN_TRUE,
)
from aorta.cia.watch.watcher import healthy_question, signal_question
from aorta.local_classifier.corpus.proposer import (
    CLASSIFIER_CANDIDATE_QUESTION_TEMPLATE,
    CLASSIFIER_CANDIDATE_WHEN_FALSE,
    CLASSIFIER_CANDIDATE_WHEN_TRUE,
)
...
_CORPUS_PROPOSER = _SRC / "local_classifier" / "corpus" / "proposer.py"
_CORPUS_WATCH = _SRC / "local_classifier" / "corpus" / "watch.py"
_SEAM = _SRC / "local_classifier" / "predictor.py"
...
class TestEachQuestionIsWrittenOnce:
    ...
    @pytest.mark.parametrize(
        ("text", "home"),
        [
            (CLASSIFIER_MITIGATION_QUESTION, _PROPOSER),
            (CLASSIFIER_CATEGORY_QUESTION, _PROPOSER),
            (CLASSIFIER_STOP_QUESTION, _PROPOSER),
            (CLASSIFIER_STOP_WHEN_TRUE, _PROPOSER),
            (CLASSIFIER_STOP_WHEN_FALSE, _PROPOSER),
            ...
            # on. ``aorta/local_classifier/corpus/proposer.py`` records the follow-up.
            (CLASSIFIER_CANDIDATE_QUESTION_TEMPLATE, _CORPUS_PROPOSER),
            (CLASSIFIER_CANDIDATE_WHEN_TRUE, _CORPUS_PROPOSER),
            (CLASSIFIER_CANDIDATE_WHEN_FALSE, _CORPUS_PROPOSER),
            (CLASSIFIER_USEFUL_QUESTION_TEMPLATE, _LOG_FINDER),
            (CLASSIFIER_USEFUL_WHEN_TRUE, _LOG_FINDER),
            (CLASSIFIER_USEFUL_WHEN_FALSE, _LOG_FINDER),
...
class TestWatchHasOneHomeNow:
    ...
    *are* the answer space, and ``ChoiceAnswer`` re-reads a distribution in the
    ...
    def test_the_builder_labels_against_the_assembled_questions(self):
        ...
        to building its own.
        """
        from aorta.local_classifier.corpus.watch import watch_questions
...
class TestTheSeamHoldsNoQuestionText:
    """``aorta.local_classifier.predictor`` serves three tracks and must not collect phrasings.

    The chat side keeps its questions in ``aorta/chat/local_classifier/questions.py`` and the

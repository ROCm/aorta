"""``python -m aorta.local_classifier``: that its hard-coded lists have not drifted, and that it refuses.

Two of the three things tested here are about drift. The Click choices are
hard-coded in ``aorta/local_classifier/cli.py`` rather than read from the registries, for the
...
them on every ``--help``. That duplication is only safe if something fails
...
The tooling is maintainer-only rather than a product command, so it is reached
as a module entry point and is absent from the ``aorta`` console script. The
subprocess tests run it the way a maintainer does, so a broken ``__main__`` or
a lost exit code fails here.
"""
...
from aorta.local_classifier.cli import _BACKENDS, _CORPUS_KINDS, cli
...
from aorta.local_classifier.predictor import CHECKPOINTS, Noul

_ENTRY_POINT = (sys.executable, "-m", "aorta.local_classifier")
...
class TestEntryPoint:
    def test_it_is_not_a_top_level_aorta_command(self):
        """Maintainer tooling; ``aorta --help`` must not list it under any name."""
        from aorta.cli import _COMMANDS

        assert not {name for name in _COMMANDS if "classifier" in name}
        assert not any(
            entry.import_path.startswith("aorta.local_classifier")
            for entry in _COMMANDS.values()
        )

    def test_the_module_help_lists_both_commands(self):
        result = _run("--help")
        assert result.returncode == 0, result.stderr
        assert "Usage: python -m aorta.local_classifier" in result.stdout
        assert "corpus" in result.stdout
        assert "eval" in result.stdout

    def test_an_empty_build_exits_non_zero_through_the_module(self, tmp_path: Path):
        """The exit code has to survive ``__main__``, or the gate is a paragraph."""
        result = _run("corpus", "watch", "--root", str(tmp_path / "absent"))
...
class TestCorpusCommand:
    def test_an_empty_build_exits_non_zero(self, tmp_path: Path):
        ...
        result = CliRunner().invoke(
            cli, ["corpus", "watch", "--root", str(tmp_path / "absent")]
        )
    ...
    def test_the_proposer_corpus_refuses_to_guess_a_root(self):
        ...
        result = CliRunner().invoke(cli, ["corpus", "proposer"])
    ...
    def test_an_unknown_kind_is_rejected_by_the_choice(self):
        result = CliRunner().invoke(cli, ["corpus", "autopsy"])
        assert result.exit_code != 0
...
class TestEvalCommand:
    ...
    def test_it_refuses_the_fake_predictor_through_the_module(self, tmp_path: Path):
        result = _run("eval", "--corpus", str(_corpus(tmp_path / "c.jsonl")), "--backend", "fake")
        assert result.returncode != 0
        assert "hash" in result.stderr

    def test_a_missing_corpus_says_how_to_build_one(self, tmp_path: Path):
        result = CliRunner().invoke(
            cli, ["eval", "--corpus", str(tmp_path / "absent.jsonl"), "--backend", "fake"]
        )
        assert result.exit_code != 0
        assert "python -m aorta.local_classifier corpus" in result.output

    def test_a_missing_local_classifier_install_reads_as_advice_not_a_traceback(self, tmp_path: Path):
        ...
        if importlib.util.find_spec("laya") is not None:
            pytest.skip("the local-classifier extra is installed, so there is no missing extra to report")
        result = CliRunner().invoke(
            cli,
            [
                "eval",
        ...
        assert result.exit_code != 0
        assert "amd-aorta[local-classifier]" in result.output

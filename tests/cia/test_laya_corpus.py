...
from aorta.laya.corpus.watch import (
    CATEGORY_TO_SIGNAL,
...
class TestTheSignalVocabularies:
    ...
    def test_the_builder_keeps_no_copy_of_the_question_text(self):
        ...
        from aorta.cia.watch.watcher import LAYA_HEALTHY_QUESTION, LAYA_SIGNAL_QUESTION
        from aorta.laya.corpus import watch as module
        ...
        assert LAYA_HEALTHY_QUESTION not in literals
        assert LAYA_SIGNAL_QUESTION not in literals
...
class TestProposerCorpus:
    ...
    def test_the_questions_come_from_the_module_that_owns_the_decision(self):
        ...
        from aorta.laya.corpus import proposer as module

        assert module._STOP.question == LAYA_STOP_QUESTION
        assert module._STOP.when_true == LAYA_STOP_WHEN_TRUE
        assert module._STOP.when_false == LAYA_STOP_WHEN_FALSE
        source = Path(module.__file__).read_text(encoding="utf-8")
        for canonical in (LAYA_MITIGATION_QUESTION, LAYA_CATEGORY_QUESTION, LAYA_STOP_QUESTION):
    ...
    def test_the_emitted_examples_carry_the_canonical_questions(self, agent_run):
        ...
        assert asked["proposer_mitigation"] == LAYA_MITIGATION_QUESTION
        assert asked["proposer_category"] == LAYA_CATEGORY_QUESTION
        assert asked["proposer_stop"] == LAYA_STOP_QUESTION
    ...
    def test_the_seam_holds_none_of_the_question_text(self):
        ...
        from aorta.agent.llm import LAYA_MITIGATION_QUESTION, LAYA_STOP_QUESTION
        from aorta.laya import predictor

        source = Path(predictor.__file__).read_text(encoding="utf-8")
        assert LAYA_MITIGATION_QUESTION not in source
        assert LAYA_STOP_QUESTION not in source
...
class TestLogFinderCorpus:
    ...
    def test_it_says_which_extra_when_the_question_cannot_be_reached(self, monkeypatch):
        ...
        from aorta.laya.corpus.log_finder import _useful_question
    ...
    def test_without_the_cia_extra_it_says_which_extra(self, monkeypatch):
        ...
        from aorta.laya.corpus.log_finder import _dir_listing
...
class TestRoundTrip:
    ...
    def test_a_missing_corpus_says_how_to_build_one(self, tmp_path: Path):
        with pytest.raises(CorpusError, match="aorta laya corpus"):

...
class TestAnswers:
    def test_a_noul_carries_a_probability_and_no_verdict(self):
        ...
        ``confidence_threshold`` escalates *above* it and a Laya clean-gate skips
...
class TestFake:
    ...
    def test_a_choice_answer_covers_exactly_the_options_offered(self):
        ...
        answer = ask_choice(FakeLayaPredictor(), "listing", _SIGNAL)
    ...
    def test_it_is_deterministic_across_processes(self):
        ...
        """
        first = ask_noul(FakeLayaPredictor(), "same log", _CLEAN).probability
        second = ask_noul(FakeLayaPredictor(), "same log", _CLEAN).probability
        assert first == second

    def test_different_states_get_different_answers(self):
        one = ask_noul(FakeLayaPredictor(), "log one", _CLEAN).probability
        two = ask_noul(FakeLayaPredictor(), "log two", _CLEAN).probability
        assert one != two

    def test_a_pinned_answer_wins(self):
        """How a later track's test controls the model without owning a checkpoint."""
        predictor = FakeLayaPredictor(pinned={_CLEAN.question: NoulAnswer(probability=0.99)})
        assert ask_noul(predictor, "anything", _CLEAN).probability == 0.99

    def test_no_questions_costs_no_answers(self):
        assert FakeLayaPredictor().ask(["a"], []) == [[]]
    ...
    def test_its_calibration_is_unknown_rather_than_clean(self):
        ...
        calibration = FakeLayaPredictor().calibration()
    ...
    def test_it_cannot_count_tokens(self):
        ...
        ``aorta.laya.gate.token_census`` looks for this method and refuses
        without it, so the absence is load-bearing rather than an omission.
        """
        assert not hasattr(FakeLayaPredictor(), "token_count")


class TestHelpers:
    def test_ask_one_returns_the_answers_for_a_single_state(self):
        answers = ask_one(FakeLayaPredictor(), "log", [_CLEAN, _SIGNAL])
        assert len(answers) == 2
...
class TestTheVerifiedRelease:
    ...
    def test_the_extra_floors_at_the_release_that_was_verified(self, repo_root):
        ...
        assert f'"laya>={VERIFIED_LAYA_VERSION}"' in pyproject, (
            f"the [laya] extra does not floor at {VERIFIED_LAYA_VERSION}, which is the "
            "release aorta/laya/predictor.py was read against. Re-read the wheel before "
    ...
    def test_the_floor_is_a_floor_and_not_a_pin(self):
        ...
        same reason.
        """
        import aorta.laya.predictor as module
...
class TestFactory:
    def test_fake_is_the_default_and_needs_nothing_installed(self):
        ...
        assert isinstance(make_predictor(), FakeLayaPredictor)
        assert isinstance(make_predictor("fake"), FakeLayaPredictor)
    ...
    def test_an_unknown_backend_lists_the_known_ones(self):
        with pytest.raises(ValueError, match="unknown Laya backend"):
    ...
    def test_importing_the_seam_pulls_in_no_model_machinery(self):
        ...
        probe = (
            "import sys, json, aorta.laya, aorta.laya.eval, aorta.laya.gate, "
            "aorta.laya.corpus;"
            f"heavy={heavy!r};"
        ...
        assert json.loads(out.stdout) == [], (
            f"importing aorta.laya pulled in {out.stdout.strip()}. Every model import "
    ...
    def test_importing_the_corpus_builders_does_not_pull_in_the_agents_extra(self):
        ...
        module scope. Reaching either eagerly would make the whole ``aorta.laya``
        ...
        probe = (
            "import sys, json, aorta.laya.corpus, aorta.laya.corpus.watch, "
            "aorta.laya.corpus.proposer;"
            "print(json.dumps(sorted(m for m in sys.modules "

...
def create_final_html(plot_file_path, output_path) :
            ...
            "gpu_time_change_percentage_summaryby_rank":
                {
                    ...
                    'chart_path' : plot_file_path / 'gpu_time_change_percentage_summaryby_rank.png'
                }
            }
    ...
    for item in nccl_chart_item_list.keys():
        nccl_image_data = get_image_data(plot_file_path / f'{item.replace(" ", "_")}_comparison.png')
...
def main():
    parser = argparse.ArgumentParser(description='Create a final HTML file for the analysis report.')
    parser.add_argument('-p','--plot-files-directory', type=Path, required=True, help='Path to the plot files direcotry.')
    parser.add_argument('-o','--output-html', type=None, default=None, help='Path to the output file.')
    args = parser.parse_args()
    output_path = args.output_html if args.output_html else args.plot_files_directory.parent / 'final_analysis_report.html'

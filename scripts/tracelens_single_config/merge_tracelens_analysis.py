...
def plot_nccl_data_per_msg(df, labels, output_dir:Path):
    ...
    vibrant_colors = ['#E63946', '#2A9D8F', '#E9C46A', '#264653', '#F4A261', '#8338EC', '#06D6A0', '#FF006E']
...
def main():
    ...
    parser.add_argument('--output', type=Path, default='./output.xlsx', help="Output xls file name")
    ...
    for directory in args.inputs:
        ...
        nccl_excel_file = dir_path / "tracelens_analysis" / "collective_reports" / "collective_all_ranks.xlsx"
        ...
        summary = summary.rename(columns={col: f'{col}_{label}' for col in summary.columns if col != 'type'})
        gpu_time = gpu_time.rename(columns={col: f'{col}_{label}' for col in gpu_time.columns if col != 'type'})
        ...
        if(nccl_excel_file.exists()):
            ...
            nccl_df['index'] = nccl_df['Collective name'].astype(str) + '_' + nccl_df['In msg nelems'].astype(str)
            
            # Rename non-key columns with label suffix (exclude 'index' as it's the merge key)
            nccl_df = nccl_df.rename(columns={col: f'{col}_{label}' for col in nccl_df.columns if col != 'index'})
    ...
    for i in range(1, len(summary_dfs)):
        ...
        gpu_time_per_rank_df = pd.merge(gpu_time_per_rank_df, gpu_time_per_rank_dfs[i], on='type', how='outer')
    ...
    cmd = ["python3", str(html_script_path), "--plot-files-directory", str(output_dir), "--output-html", str(args.output.parent / 'final_analysis_report.html')]
    if(run_command(cmd, "Creating final HTML")) : 
        print(f"Final HTML file created at: {args.output.parent / 'final_analysis_report.html'}")
    else:

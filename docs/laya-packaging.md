# Decision 22 — where the Laya weights may live, and which weights answered

This is the packaging decision for the Laya integration planned in
[`docs/plans/laya-system1-integration.md`](plans/laya-system1-integration.md). It
...
The Phase 0 and Phase 1 offline tooling has since landed — the `[laya]` extra,
`aorta laya`, and `src/aorta/laya/` — and this record has been reconciled against
...
1. **`torch` may be declared by the opt-in `[laya]` extra and by nothing else.
...
2. **A checkpoint is pinned by digest, never by name or tag, and every report a
   Laya verdict reaches records which checkpoint, which library version, and
...
| `_HEAVY_PREFIXES` in [`tests/cli/test_chat_boundaries.py`](../tests/cli/test_chat_boundaries.py), via `test_importing_aorta_cli_does_not_pull_in_langchain` | the `import aorta.cli` graph | an import moved to module scope in `aorta/cli/chat.py`, and now also in `aorta/cli/laya.py` |
...
| `test_importing_the_seam_pulls_in_no_model_machinery` in [`tests/cia/test_laya_predictor.py`](../tests/cia/test_laya_predictor.py) | the `import aorta.laya` graph, including `.eval`, `.gate` and `.corpus` | `torch`, `transformers`, `laya`, `onnxruntime`, `numpy` or `safetensors` reaching module scope in the seam itself |
...
Registering `aorta laya` as a `LazyCommand` in `aorta/cli/__init__.py` means the
second row now covers the new command module for free: `torch` is already in
`_HEAVY_PREFIXES`, so an eager import in `aorta/cli/laya.py` fails an existing
test rather than needing a new one.
...
where `LayaProposer` goes. That test exists because
...
So the dependency goes in its own `[laya]` extra, and
...
`[laya]` declares `laya>=0.3.4`, and the floor is worth a paragraph because it
does not describe what an install actually gets.
...
Two options were considered for getting a torch-free Laya into `aorta chat`.
...
| --- | --- | --- | --- |
| `aorta laya` corpus and eval (Phases 0–1, landed) | `[laya]` | torch | inside the method that loads a checkpoint; `aorta.laya` has its own probe |
| Watch clean-gate (Track A) | `[laya]` | torch | inside `LogWatcher.forward`'s tier, not at module scope |
| `LayaProposer` (Track B) | `[laya]` | torch | inside `propose()`; `aorta.agent.llm` is import-gated by the boundary test |
| `LogFinder` tier 3 (Track C) | `[laya]` | torch | inside the tier |
| Autopsy confidence (Phase 4) | `[laya]` | torch | inside the router call |
...
- **Record the temperature fit with it.** The calibration is not in the weights.
  One temperature per (question type, option count) is what moves mean ECE from
  0.466 to 0.081, so a probability is a function of the checkpoint *and* the fit
...
- **Never fetch a checkpoint implicitly at first use on a customer node.** The
  weights are ~808 MB and `[laya]` will be installed beside `[cia]` on nodes that
  may have no egress.
...
uninformative. `src/aorta/laya/predictor.py` therefore does not surface the field
...
and `confidence_from_probs`, and `src/aorta/laya/eval.py` deliberately reimplements
